...
const AppConfig = (() => {
  ...
  const setUriTemplates = (str: string = ''): string | null => {
    ...
    for (let n = 0; n < imgs.length; n++) {
      ...
      str = str.replace(`${imgTemplate}"${imgs[n]}"`, `src="${getAkamayUrl(src)}"`)
    }
    ...
    for (let n = 0; n < anchors.length; n++) {
      ...
      str = str.replace(`${anchorTemplate}"${anchors[n]}"`, `href="${href}" target="_blank"`)
    }
  ...
  const decodeHTMLEntities = (str: string) => {
    let textArea = globalThis?.window?.document?.createElement('textarea')
  ...
  const styledHTMLText = (str: string) => {
    ...
    convertedEntities = convertedEntities.replace(/<\/iframe>/g, '</iframe></div>')
    convertedEntities = convertedEntities.replace(
      /<table>/g,
      "<div class='longtext-table' style='overflow-x: auto;'><table>"
    )
    convertedEntities = convertedEntities.replace(/<\/table>/g, '</table></div>')
