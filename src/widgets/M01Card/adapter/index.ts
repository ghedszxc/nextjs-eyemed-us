...
export class M01CardAdapter extends Adapter<TM01Card, Nullable<TM01Card>> {
  adapt: (source: any) => Nullable<TM01Card> = source => {
    ...
    const ctas = getAdapterCTA(data?.teaserTargets)?.map(teaserTarget => ({
      href: teaserTarget?.url || '',
      text: teaserTarget?.label || '',
      isExternal: teaserTarget?.isExternal || false,
      ctaStyle: teaserTarget?.ctaStyle || 'default',
    }))
