...
export const getAdapterCTA = (obj: IAdapterCTAObj[]): ICta[] => {
  return obj?.map((link: IAdapterCTAObj): ICta => {
    ...
    return {
      label: link?.callToActionText || link?.target?.teaserText || link?.target?.title || '',
      url: isExternal
        ? `${link?.target?.url || ''}`
...
export const getCoreMediaUrl = (str: string) => {
  ...
  const url = str.replace(prefix + '/', '').replace(prefix, '')
  // let pathname = localeSegmentRemoval(url)
  let pathname = url
  ...
  return internalLink ? `${origin}${pathname.charAt(0) === '/' ? '' : '/'}${pathname}` : pathname
}
